...
# Under decision: Under 2.5 vs 3.5 usando proxy: somma G3+ (fatti team + concessi opp)
team_g3p = team_dist["G3"] + team_dist["G4+"]
opp_g3p_conc = g3pc

under_ok = (team_g3p < 0.25) and (opp_g3p_conc < 0.20)
if under_ok:
    under_choice = "Under 2.5" if (team_dist["G0"] + team_dist["G1"] >= 0.45) else "Under 3.5"
    badge="🟢"
    alt_results.append({"name": under_choice, "label":"ROBUSTO", "badge":badge, "lose_if":"3+ gol totali" if under_choice=="Under 2.5" else "4+ gol totali", "kind":"UNDER"})

# BTTS NO proxy: rischio 0 gol team (G0) o opp scoring? qui usiamo G0 subiti opp e G0 fatti team
btts_no_ok = (team_dist["G0"] >= 0.22) or (g0c >= 0.30)
if btts_no_ok:
